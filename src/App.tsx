md"> <div className="flex gap-2"> <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Location (e.g. Enugu)" className="w-full rounded p-3" /> <input value={maxPrice} onChange={(e) => setMaxPrice(e.target.value === "" ? "" : Number(e.target.value))} placeholder="Max ₦" className="w-28 rounded p-3" /> <button type="submit" className="rounded bg-indigo-700 px-4 py-2 text-white">Search</button> </div> </form> </div> </section>
    ...
    <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-6 p-4">
        ...
        <nav className="flex items-center gap-4">
          ...
          <Link to="/admin" className="rounded border px-3 py-1 text-sm">
            Admin
          </Link>
          <Link
            to="/login"
            ...
            <article key={p.id} className="rounded border p-3">
              ...
              <div className="mt-2 flex items-center justify-between">
                ...
                <div className="text-right">
                  <div className="font-bold">{currency(p.pricePerNight)}</div>
                  <button
                    onClick={() => window.location.assign(`/property/${p.id}`)}
...
function PropertiesPage() {
  const [propsList, setPropsList] = useState(sampleProperties);

  useEffect(() => {
    // Could parse search params to filter
  }, []);
...
function BookingPage() {
  ...
  function onPay() {
    ...
    setTimeout(() => {
      setStatus("paid");
      alert("Payment simulated: booking confirmed (placeholder)");
    }, 1200);
  }
...
function LoginPage() {
  ...
  function onLogin(e: React.FormEvent) {
    ...
    localStorage.setItem(
      "ss_user",
      JSON.stringify({ email, role: "guest" })
    );
    navigate("/dashboard");
  }
...
function Dashboard() {
  ...
  const bookings = [
    {
      id: "b1",
      propertyName: "Enugu Royal Hotel",
      checkIn: "2025-10-01",
      checkOut: "2025-10-04",
      status: "upcoming",
    },
  ...
  return (
    <main className="mx-auto max-w-4xl p-6">
      ...
      <div className="mt-4">
        ...
        {bookings.map((b) => (
          <div key={b.id} className="mt-2 rounded border p-3">
            <div className="flex items-center justify-between">
              <div>
